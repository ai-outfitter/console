...
export const resolveConfigPath = (): string => {
  const explicit = process.env.OUTFITTER_CONSOLE_CONFIG ?? process.env.A2ASTRO_CONFIG;
  if (explicit) return resolve(explicit);
  const candidates = [
    'console.config.yaml',
    'console.config.mock.yaml',
    'console.config.example.yaml',
    // Compatibility during migration from ncrmro/a2astro.
    'a2astro.config.yaml',
